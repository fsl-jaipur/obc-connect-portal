...
function Dropdown({ label, placeholder, value, options, onSelect, disabled }: DropdownProps) {
  ...
  return (
    <div ref={ref} className="relative flex flex-col">
      <label className="text-sm font-semibold text-[#0f2056] mb-1">{label}</label>
      <button
        type="button"
        disabled={disabled}
        onClick={() => {
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 z-50 mt-2 rounded-lg border border-gray-200 bg-white shadow-lg overflow-hidden">
          ...
          <div className="max-h-60 overflow-auto">
            {filtered.length === 0 ? (
              <div className="px-4 py-3 text-sm text-gray-500">कोई परिणाम नहीं मिला</div>
            ) : (
              filtered.map((o) => (
                <button
                  key={o}
                  ...
                  className={`w-full text-left px-4 py-2 text-sm transition ${
                    o === value ? "bg-orange-50 text-[#0f2056] font-semibold" : "hover:bg-gray-50 text-gray-700"
                  }`}
                >
                  {o}
                </button>
